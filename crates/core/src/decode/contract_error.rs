...
pub async fn resolve(
    ...
    let wasm_bytes = if let Some(cached) = cache.get(crate::cache::store::CacheCategory::WasmBlob, &cache_key)? {
        cached
    } else {
        let wasm = fetch_contract_wasm(contract_id, network).await?;
        let _ = cache.put(crate::cache::store::CacheCategory::WasmBlob, &cache_key, &wasm);
        wasm
    };
...
async fn fetch_contract_wasm(
    ...
    let _result = rpc
        .get_ledger_entries(&[contract_id.to_string()])
        .await?;
